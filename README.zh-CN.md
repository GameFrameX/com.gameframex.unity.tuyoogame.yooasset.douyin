...
Game Frame X YooAsset DouYin 是面向 Unity WebGL **抖音小游戏（大陆版）** 运行时的 YooAsset `IFileSystem` 适配包。它将 ByteGame 平台特有的文件系统行为接入 YooAsset 资源管线，让你的项目能够通过抖音小游戏平台分发 AssetBundle。

## 功能特性

- 为抖音小游戏运行时提供专用的 `ByteGameFileSystem` 实现。
- 与抖音小游戏 SDK 深度集成，覆盖 AssetBundle 下载、缓存与设备端读取。
- 打通 YooAsset 的包版本请求、清单加载与资源包下载/加载全流程。
- 支持可插拔的 `IRemoteServices` 接入点，便于自定义 CDN 与解密逻辑。
...
3. 在 Unity 的 **Package Manager** 中使用 **Git URL** 的方式添加库，地址为：`https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.douyin.git`
`
...
安装完成后，将 ByteGame 文件系统参数交给 YooAsset 的 Web 模式初始化器即可：
...
查看 [Releases](https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.douyin/releases) 了解更新日志。
